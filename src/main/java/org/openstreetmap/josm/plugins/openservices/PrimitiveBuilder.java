...
/**
 * The PrimitiveBuilder provides methods to create josm primitives from 
 * JTS geometries and add them to a josm dataset.
 * The JTS geometries must be in the josm crs (epsg:4326)
 * The methods take care of removal of duplicate nodes in ways, and merging
...
//    }
  ...
  public Collection<OsmPrimitive> build(Polygon polygon, Map<String, String> keys) {
   ...
   */
  private OsmPrimitive buildPolygon(Polygon polygon, Map<String, String> keys) {
  ...
  public Node buildNode(Coordinate coordinate, boolean merge) {
    LatLon latlon = new LatLon(coordinate.x, coordinate.y);
